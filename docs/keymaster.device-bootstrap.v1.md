...
同一条 `location` 只允许登记一次连接:

1. 先按上表规范化(endpoint 去尾斜杠、prefix 去首尾 `/` 等)。
2. 把对象的键名按字典序排序(递归),`JSON.stringify` 得到规范形式。
3. 两条连接的规范形式不得相同。

位置指纹不落盘:需要比较时读取方自行计算。
...
```jsonc
{
  "keyDerivation": {
    "iterations": 600000,
    "saltB64Url": "pQ7xW1sT4hJ6pA0uZ5yE2oI9gK3rM7qX1"
  },
  "cipher": {
    "ivB64Url": "gT3kQ1sPvJ0mYw",
    "ciphertextAndTagB64Url": "kQ9fLx2mR7dW3cV1nB8sT4hJ6pA0uZ5yE2oI9gK3rM7qX1tC"
  }
}
```
...
| `saltB64Url` | 字符串 | 是 | 公开随机盐,Base64URL,最长 128 字符 |

`cipher` — 用派生密钥加密的配置密文:

| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `ivB64Url` | 字符串 | 是 | 随机 nonce,Base64URL(`A-Za-z0-9_-`) |
| `ciphertextAndTagB64Url` | 字符串 | 是 | 密文与标签拼接,Base64URL;字符串长度 ≤ 32768 |
...
5. **唯一性**:
   ...
   - 规范化 `location` 在 `connections` 内唯一。
